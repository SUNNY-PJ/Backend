...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long>, ChatMessageRepositoryCustom {
	...
	@Query(value =
		"select A.chat_room_id, u.user_id as userFriendId, u.name as friendName, u.profile as friendProfile, A.readCnt, C.message "
			+ "from chat_user cu "
			...
			+ "where cu.user_id = :id and cm.user_id != :id and cm.read = 1 "
			+ "group by cm.chat_room_id) A on A.chat_room_id = cu.chat_room_id "
			+ "join ( "
			+ "select * "
			+ "from ( "
			+ "select *, ROW_NUMBER() over (PARTITION by cm.chat_room_id ORDER by cm.created_date) as row_num "
