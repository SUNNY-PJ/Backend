...
public class CommentService {
	...
	public ResponseEntity<CommonResponse.SingleResponse<CommentResponse>> createComment(
		...
		return responseService.getSingleResponse(HttpStatus.OK.value(),
			new CommentResponse(comment.getId(), comment.getUsers().getId(), comment.getUsers().getName(),
				comment.getContent(),
	...
	private void replySendNotifications(CustomUserPrincipal customUserPrincipal, Users users,
		...
		if (notificationList.size() != 0) {
			NotificationPushRequest notificationPushRequest = new NotificationPushRequest(postAuthor, bodyTitle, body);
